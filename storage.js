...
export async function initStorage() {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS oracle_route_performance (
      ...
      feedback_total DOUBLE PRECISION NOT NULL DEFAULT 0,
...
export async function loadRoutePerformance() {
  ...
  const result = await pool.query(`
    SELECT route_key, model_id, domain, depth, attempts, successes, failures, repairs,
           feedback_total, feedback_count, avg_ms
    FROM oracle_route_performance
  `);
  return result.rows;
}
...
export async function saveRoutePerformance({ routeKey, modelId, domain, depth, performance }) {
  if (!enabled) return;
  await pool.query(`
    INSERT INTO oracle_route_performance (
      route_key, model_id, domain, depth, attempts, successes, failures, repairs,
      feedback_total, feedback_count, avg_ms, updated_at
  ...
  `, [
    ...
    performance.attempts, performance.successes, performance.failures, performance.repairs,
    performance.feedbackTotal, performance.feedbackCount, performance.avgMs
  ]);
}
...
export async function getUsageSummary(days = 30, apiKeyId = null) {
  ...
  const result = await pool.query(`
    SELECT
      ...
      COALESCE(AVG(feedback_score), 0)::double precision AS avg_feedback,
