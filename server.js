...
async function fetchWithTimeout(url, options) {
  ...
  catch (error) { if (error?.name === "AbortError") throw new Error(`Provider timed out after ${PROVIDER_TIMEOUT_MS}ms`); throw error; }
  finally { clearTimeout(timer); }
}
async function callOpenAI({ model, prompt, effort = "low", maxOutputTokens = 2200 }) {
  if (!process.env.OPENAI_API_KEY) throw new Error("OPENAI_API_KEY is not configured.");
  const response = await fetchWithTimeout("https://api.openai.com/v1/responses", { method: "POST", headers: { Authorization: `Bearer ${process.env.OPENAI_API_KEY}`, "Content-Type": "application/json" }, body: JSON.stringify({ model, input: prompt, reasoning: { effort }, max_output_tokens: maxOutputTokens, store: false }) });
...
async function callProvider({ provider, model, prompt, effort = "low", maxOutputTokens = 2200 }) {
  ...
  try { if (provider === "openai") result = await callOpenAI({ model, prompt, effort, maxOutputTokens }); else if (provider === "anthropic") result = await callAnthropic({ model, prompt, maxOutputTokens }); else if (provider === "gemini") result = await callGemini({ model, prompt, maxOutputTokens }); else throw new Error(`Unsupported provider: ${provider}`); }
...
async function runCandidate({ request, route, model }) {
  ...
  try { let executed = await execute({ request, route, executionModel: model }); calls.push(executed); let judged = await judgeAnswer({ request, route, answer: executed.text }); calls.push(judged.call); let answer = executed.text;
    if (!judged.qa.pass && judged.qa.repair_instructions) { const repairCall = await execute({ request, route, executionModel: model, repair: judged.qa.repair_instructions }); calls.push(repairCall); repaired = true; metrics.repairs++; answer = repairCall.text; judged = await judgeAnswer({ request, route, answer }); calls.push(judged.call); }
    const elapsedMs = Date.now() - started; const pass = Boolean(judged.qa.pass); updatePerformance({ modelId: model.id, domain: route.domain, depth: route.depth, success: pass, repaired, ms: elapsedMs }); return { ok: pass, answer, qa: judged.qa, repaired, calls, elapsedMs, error: pass ? null : "QA failed after repair" };
...
app.get("/api/health", (_req, res) => res.json({ ok: true, service: "oracle-stack", mode: "adaptive-multi-model-execution", routingPolicy: ROUTING_POLICY, persistence: storageEnabled() ? "postgres" : "memory", developerApi: apiAuthEnabled() ? "enabled" : "disabled", failover: { enabled: true, maxModels: MAX_FAILOVER_MODELS, providerTimeoutMs: PROVIDER_TIMEOUT_MS }, providers: [...new Set(modelRegistry().map(model => model.provider))], models: modelRegistry().map(model => ({ id: model.id, provider: model.provider, model: model.model })) }));
...
async function oracleHandler(req, res) {
  ...
  try { const request = String(req.body?.request || "").trim(); const requestedModel = String(req.body?.model || "").trim(); if (!request) return res.status(400).json({ error: "Request is required." }); if (request.length > 12000) return res.status(400).json({ error: "Request is too long for V2. Keep it under 12,000 characters." });
    ...
    for (let index = 0; index < candidates.length; index++) { const model = candidates[index]; metrics.byModel[model.id] = (metrics.byModel[model.id] || 0) + 1; const candidateResult = await runCandidate({ request, route, model }); attempts.push({ modelId: model.id, provider: model.provider, model: model.model, routingScore: model.routingScore ?? null, ok: candidateResult.ok, repaired: candidateResult.repaired, elapsedMs: candidateResult.elapsedMs, error: candidateResult.error }); if (candidateResult.ok) { result = candidateResult; finalModel = model; break; } if (index < candidates.length - 1 && decision.allowFailover) metrics.failovers++; }
    ...
    const telemetry = { executionId, apiKeyId, elapsedMs, modelCalls: allCalls.length, inputTokens, outputTokens, totalTokens, specialist: route.domain, depth: route.depth, repaired: result.repaired, provider: finalModel.provider, model: finalModel.model, modelId: finalModel.id, routingPolicy: ROUTING_POLICY, routingReason: attempts.length > 1 ? `${decision.reason}_failover` : decision.reason, routingScore: finalModel.routingScore ?? null, failoverCount: Math.max(0, attempts.length - 1), attemptedModels: attempts };
