...
import {
  ...
  saveExecution, findExecution, saveFeedback, getUsageSummary, listArtifacts, loadArtifactBundle, artifactLineage, latestPassingArtifact, saveBetaRequest
} from "./storage.js";
...
app.post("/api/beta-request", async (req,res) => { const email=String(req.body?.email||"").trim().toLowerCase(), name=String(req.body?.name||"").trim().slice(0,120), useCase=String(req.body?.useCase||"").trim().slice(0,1500); if(!/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(email)) return res.status(400).json({error:"A valid email is required."}); try { const request=await saveBetaRequest({email,name,useCase}); res.status(201).json({ok:true,status:request.status,message:"Beta access request received."}); } catch(error) { console.error("Beta request failed:",error.message); res.status(503).json({error:"Beta request storage is temporarily unavailable."}); } });\napp.get("/api/models", (_req, res) => { const models = modelRegistry().map(({ id, provider, model, strengths, quality, speed, cost }) => ({ id, provider, model, strengths, quality, speed, cost })); res.json({ policy: ROUTING_POLICY, models }); });
